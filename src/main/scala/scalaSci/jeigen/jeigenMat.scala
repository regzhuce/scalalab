 ...
 */
 ...
 override final def * (that: RichDouble2DArray): Mat =  {
   var rdm = new jeigen.DenseMatrix(that.Nrows, that.Ncols)
   var r=0; var c=0
   while (r<that.Nrows) {
     c = 0
     while (c<that.Ncols) {
         rdm.set(r,c, that(r,c))
         c += 1
      }
      r += 1
    }
   
   new Mat(this.dm.mmul(rdm))
  }
...
final def mexp() = {
  ...
  final def ldltSolve(b: Mat) = {
     new Mat(dm.ldltSolve(b.dm))
  }
