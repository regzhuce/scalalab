...
class Mat(var da: Array[Array[Double]])  extends AnyRef with scalaSci.scalaSciMatrix[scalaSci.JBLAS.Mat]  { 
  ...
  final def getNativeMatrixRef() =   dm // the scalaSci.JBLAS.Mat class wraps the org.jblas.DoubleMatrix  class, thus return simply the data representation

  final def matFromNative() = new Mat(dm)
  final def matFromNative(dm: DoubleMatrix) = new Mat(dm)
