...
public class Evaluator {
    ...
    private void initializeEvaluator(EvaluationContext evaluationContext) {
        ...
        try {
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.ARPACKFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.jarFilePath));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.ejmlFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.jblasFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.jsciFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.mtjColtSGTFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.ApacheCommonsFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.numalFile));
                   classLoader.addURL(new URL("file", "localhost", scalalab.JavaGlobals.LAPACKFile));
          ...
          if (toolboxesFolderFiles!=null) {  // DefaultToolboxes folder not empty
           ...
           for (int f=0; f < numFiles;  f++) {
               String currentFileName = toolboxesFolderFiles[f].getAbsolutePath();
                  ...
                  classLoader.addURL(new URL("file", "localhost", currentFileName));
                 System.out.println("JavaREPL appending from defaultToolboxes folder toolbox: "+currentFileName);
               }  // endsWith("jar")
