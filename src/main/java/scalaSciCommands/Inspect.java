...
for (int k=0; k<classMethods.length; k++) {
        ...
        public static void inspectCompletionList(Object object)
   {
       ...
       //    var x = new org.apache.commons.math4.distribution.NormalDistribution
...
clList.addKeyListener(new KeyListener() {
     ...
     public static void inspectCompletionListREPL(Sequence<javarepl.completion.CompletionCandidate> candidates)
   {
     ...
     int numCompletionsItems = candidates.size();
     
    
     
     candidates.forEach(new Callable1() {

         @Override
         public Object call(Object input) throws Exception {
            //System.out.println("adding elemenet "+input.toString());
    String currentElement = ((CompletionCandidate)input).value;
    dcl.addElement(currentElement);
    scanMethodsFields.add(currentElement);
    return input;
   ...
   JFrame completionFrame  = new JFrame("Completion List ");
        
       
    JScrollPane  completionPane = new JScrollPane(clList);
   completionFrame.add(completionPane);                                                       // in order to approximate the vertical size of the completion list
   
    int    approxPixelsPerItem = 20;    // approx. how many pixels to take vertically for a list item
    completionFrame.setSize(600,  numCompletionsItems*approxPixelsPerItem);
    ...
    if (scalaExec.Interpreter.GlobalValues.completionIsForSyntaxPane == true)  // completion is for JSyntaxPane editor (in order to display the completion popup properly)
          ...
          SwingUtilities.convertPointToScreen(p, scalaExec.Interpreter.GlobalValues.globalRSyntaxEditorPane);
            ...
            p.x = p.x + 2;
            p.y = p.y + 20;
            completionFrame.setLocation(p.x, p.y);
